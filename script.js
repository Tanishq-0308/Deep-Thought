...
const assetBox = document.querySelectorAll(".asset");
let isOpen = false;
...
// here is put the assets array in a new variable
const newContent = details.tasks[0].assets;
assetBox.forEach((div, index) => {
  // Select the h2 and p elements within this div
  const h4 = div.querySelector("h4");
  const p = div.querySelector("p");

  // Change their content
  if (h4 && p && newContent[index]) {
    h4.innerHTML = `Description: <span>${newContent[index].asset_description}</span>`;
    p.innerHTML = `${newContent[index].asset_title} <span class="infor"><i class="ri-information-line info"></i></span>`;
  }
});
taskTitle.innerHTML = details.tasks[0].task_title;
taskDescription.innerHTML = details.tasks[0].task_description;

arrow.addEventListener("click", () => {
  if (!isExpanded) {
    journeyBoard.style.left = "-170px";
    arrow.style.transform = "rotate(180deg)";
    journeyClose.style.display = "none";
    journeyOpen.style.display = "block";
    isExpanded = true;
  } else {
    journeyBoard.style.left = "-400px";
    journeyOpen.style.display = "none";
    journeyClose.style.display = "flex";
